...
at::Tensor dcn2Dv3_cuda_forward(const at::Tensor &input, const at::Tensor &offset,
    ...
    auto output = at::zeros({batch, height_out, width_out, group * group_channels}, input.options());

    const int batch_n = im2col_step_;
    auto output_n = output.view({batch / batch_n, batch_n, height_out, width_out, group * group_channels});
...
dcn2Dv3_cuda_backward(const at::Tensor &input, const at::Tensor &offset,
    ...
    auto per_offset_size = height_out * width_out * group * kernel_h * kernel_w * 2;
    auto per_mask_size = height_out * width_out * group * kernel_h * kernel_w;
    auto grad_output_n = grad_output.view({batch / im2col_step_, batch_n, height_out * width_out, group, group_channels});
