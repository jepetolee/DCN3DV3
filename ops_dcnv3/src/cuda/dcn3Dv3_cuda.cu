...
at::Tensor dcn3Dv3_cuda_forward(const at::Tensor &input, const at::Tensor &offset, const at::Tensor &mask, 
    ...
    const int length_in = input.size(3);
    const int channels = input.size(4);
    ...
    auto per_input_size = height_in * width_in * length_in * group * group_channels;
    auto per_offset_size =
        height_out * width_out * group * kernel_h * kernel_w * kernel_l * 2;
    auto per_mask_size = height_out * width_out * group * kernel_h * kernel_w * kernel_l;
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(
            input.type(), "ms_deform_attn_forward_cuda", ([&] {
                dcn3Dv3_im2col_cuda(
                    ...
                    columns.data<scalar_t>(), kernel_h, kernel_w, kernel_l,
...
dcn3Dv3_cuda_backward(const at::Tensor &input, const at::Tensor &offset,const at::Tensor &mask, 
    ...
    auto per_mask_size = height_out * width_out * group * kernel_h * kernel_w * kernel_l;
    auto grad_output_n =
        grad_output.view({batch / im2col_step_, batch_n, height_out * width_out * kernel_l,
