...
#ifdef WITH_CUDA
        return dcn1Dv3_cuda_forward(input, offset, mask, kernel_l,
...
dcn1Dv3_backward(const at::Tensor &input, const at::Tensor &offset,
               const at::Tensor &mask, const int kernel_l,const int stride_l,
    ...
    if (input.type().is_cuda()) {
#ifdef WITH_CUDA
        return dcn1Dv3_cuda_backward(input, offset, mask, kernel_l, stride_l, 
                                     pad_l, dilation_l, dilation_l, group, group_channels,
